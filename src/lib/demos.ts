export type DemoFounder = {
  key: string;
  name: string;
  email: string;
  startup: string;
  role: string;
  image: string;
  mrrCents: number;
  payingUsers: number;
  runwayMonths: number;
  weeklyGoal: string;
  blurb: string;
  quote: string;
};

/** Demo founders used for signup page try-outs, seeding, and landing testimonials. */
export const DEMO_FOUNDERS: DemoFounder[] = [
  {
    key: "ada",
    name: "Ada Chen",
    email: "ada@demo.pulseboard.lol",
    startup: "Northwind Labs",
    role: "Founder",
    image: "/demos/ada.jpg",
    mrrCents: 184_000,
    payingUsers: 11,
    runwayMonths: 9,
    weeklyGoal: "Close two design partners",
    blurb: "B2B ops · $1.8k MRR",
    quote:
      "I used to open Stripe, a sheet, and Notion every Monday. Now it’s one page and I’m done before coffee cools.",
  },
  {
    key: "ben",
    name: "Ben Ortiz",
    email: "ben@demo.pulseboard.lol",
    startup: "Harbor AI",
    role: "Solo founder",
    image: "/demos/ben.jpg",
    mrrCents: 42_000,
    payingUsers: 3,
    runwayMonths: 14,
    weeklyGoal: "Ship the Monday digest",
    blurb: "AI notes · $420 MRR",
    quote:
      "Three priorities. That’s it. The rest of the noise finally stopped competing for a line on the board.",
  },
  {
    key: "cara",
    name: "Cara Ng",
    email: "cara@demo.pulseboard.lol",
    startup: "Relay Desk",
    role: "CEO",
    image: "/demos/cara.jpg",
    mrrCents: 96_000,
    payingUsers: 8,
    runwayMonths: 6.5,
    weeklyGoal: "Cut support backlog in half",
    blurb: "Support inbox · $960 MRR",
    quote:
      "The customer log made the week real. Names and dollars, not another vanity chart I never open.",
  },
  {
    key: "devon",
    name: "Devon Park",
    email: "devon@demo.pulseboard.lol",
    startup: "Stackline",
    role: "Co-founder",
    image: "/demos/devon.jpg",
    mrrCents: 240_000,
    payingUsers: 19,
    runwayMonths: 11,
    weeklyGoal: "Ask five users for a yearly plan",
    blurb: "Dev tools · $2.4k MRR",
    quote:
      "We run the company from this scorecard. MRR, runway, three bets — same ritual every Monday.",
  },
  {
    key: "elena",
    name: "Elena Vos",
    email: "elena@demo.pulseboard.lol",
    startup: "Monday Metric",
    role: "Founder",
    image: "/demos/elena.jpg",
    mrrCents: 12_000,
    payingUsers: 1,
    runwayMonths: 18,
    weeklyGoal: "Get paying user #2",
    blurb: "Solo founder · $120 MRR",
    quote:
      "I have one paying user. Pulseboard still helps — it keeps me honest about what has to move this week.",
  },
];

export function findDemo(key: string) {
  return DEMO_FOUNDERS.find((d) => d.key === key);
}
